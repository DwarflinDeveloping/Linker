def manage_search(message):
    ...
    for search in searches:
        if ":" in search:
            from commands.custom_family import manage_search
            search_url = manage_search(search)
        else:
            import os
            user_has_custom_family = os.path.isfile(f"data/user_familys/{str(message.author.id)}.txt")
            guild_has_custom_family = os.path.isfile(f"data/guild_familys/{str(message.guild.id)}.txt")
            search_request = search
            if guild_has_custom_family:
                ...
                if user_has_custom_family:
                    guild_family_file = open(f"data/user_familys/{str(message.author.id)}.txt", "r")
                    search_url = guild_family_file.read().replace("%ARTICLE%", search_request)
                    guild_family_file.close()
                else:
                    no_family = True
                    search_url = "https://de.wikipedia.org/wiki/%ARTICLE%".replace("%ARTICLE%", search_request)
