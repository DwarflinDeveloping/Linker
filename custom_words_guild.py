async def manage_request(ctx, *args):
    ...
    if args[0] == "get":
        ...
        if get_process == ReturnCodes.NOT_FOUND:
            await ctx.send("This guild has no custom words.")
        else:
            await ctx.send(f"The custom words of this guild are:```{get_process}```")
        ...
        if clear_process == ReturnCodes.NOT_FOUND:
            ...
            await ctx.send("The guilds custom words succesfully cleared.")

    elif args[0] == "remove":
        add_process = remove_custom_guild_word(ctx.guild.id, args[1])
        from utils import ReturnCodes
        if add_process == ReturnCodes.SUCCESS:
            ...
            await ctx.send("This guild doesn't have this custom word.")
